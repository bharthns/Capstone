...
def create_no2_map(data, title="NO2 Concentration Map", value_scale=1e15):
    """Create an interactive map visualization using plotly."""
    display_data = data / value_scale if value_scale else data
    color_label = "NO2 (×10¹⁵ mol/cm²)" if value_scale else "NO2"
    fig = px.imshow(
        display_data,
...
def build_downscaled_dataframe(data, transform, scale_factor=1, value_scale=1e15):
    """Convert a 2D grid into a tidy dataframe with coordinates."""
    rows, cols = data.shape
    ...
    if transform is not None:
        effective_transform = transform * Affine.scale(1/scale_factor)
        ...
        df_dict["latitude"] = np.array(latitudes)
        df_dict["longitude"] = np.array(longitudes)
    else:
        df_dict["latitude"] = row_flat
        df_dict["longitude"] = col_flat

    df_dict["no2_value_raw"] = values

    if value_scale:
        df_dict["no2_value_x1e15"] = values / value_scale

    return pd.DataFrame(df_dict)

def format_downscaled_csv(data, transform, scale_factor=1, value_scale=1e15):
    """Return a CSV (bytes) summarizing the downscaled grid with coordinates."""
    df = build_downscaled_dataframe(
        data=data,
        transform=transform,
